...
class TankMediator(Mediator):
    ...
    def notify(self, event:str, **kwargs):
        if event == "Shooting":
            bullet_pos = (self._physics_tank.x + self._physics_tank.wh/2 * math.cos(self._physics_tank.angle),
                          self._physics_tank.y + self._physics_tank.wh/2 * math.sin(self._physics_tank.angle))
